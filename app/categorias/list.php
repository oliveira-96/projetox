<?php
    ...
    $query = mysqli_query($db, "SELECT * FROM categorias" . (!empty($busca) ? " WHERE ({$search})" : '') . " ORDER BY id ASC LIMIT {$inicio}, {$limite}");
?>
...
<div class="row">
	<div class="col-lg-12">
                    ...
                    if (mysqli_num_rows($query)) {
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <?= (!empty($item['categoria']) ? $item['categoria'] : ''); ?>
                                </td>
                                <td style="width: 10%;">
                                    <?= (!empty($item['status']) ? 'Habilitada' : 'Desabilitada'); ?>
                                </td>
        ...
        <?php
            ...
            $query = mysqli_query($db, "SELECT count(*) AS cont FROM categorias" . (!empty($busca) ? " WHERE ({$search})" : ''));
            if ($query) {
