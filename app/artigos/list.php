<?php
    ...
    if (!empty($busca)) {
        ...
        $campos = array();
        $campos[] = "titulo LIKE '%{$search}%'";
        $campos[] = "resumo LIKE '%{$search}%'";
        $campos[] = "conteudo LIKE '%{$search}%'";
    ...
    $query = mysqli_query($db, "SELECT * FROM artigos" . (!empty($busca) ? " WHERE ({$search})" : '') . " ORDER BY id ASC LIMIT {$inicio}, {$limite}");
?>
...
<div class="row">
    <div class="col-lg-12">
        <h3 class="page-header">Artigos</h3>
    </div>
</div>
...
<div class="row">
	<div class="col-lg-12">
        ...
        <?php
            ...
            $query = mysqli_query($db, "SELECT count(*) AS cont FROM artigos" . (!empty($busca) ? " WHERE ({$search})" : ''));
            if ($query) {
